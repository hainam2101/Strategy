...
namespace Strategy {
	...
	public class SoundPlayer :ISoundStopEventReceiver , IGameMusicPlayer, IEffectPlayer {
		...
		private ISoundEngine engine; // Main object of this music player
		private const string songPath = "../../media/music"; // Path to folder witch songs (sounds) --mp3,ogg,wav
		private const string effectPath = "../../media/music/effects"; // Path to folder witch effects
		private const float volumeJump = 0.1f; // Difference to volume up/down

		private int current = 0; // Number of actual song
		private string[] songs; // Names of each song from folder
		private Dictionary<string, string> effects; // Names of each sound from folder
		private ISound sound; // Current playing song
		private Mogre.RenderWindow mWindow; // RenderWindow instance for making overlays
		private float mTimer; // Float as timer to determine of duration overlay
		private Random r; // Makes random number to Select song 
		...
		/// <summary>
		/// Changes the value of the music player volume by a given number. Contols max and min bounds
		/// of the volume are 0-1.
		/// </summary>
