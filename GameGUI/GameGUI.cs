...
namespace Strategy.GameGUI {
	...
	public class GameGUI : IGameGUI {
		...
		// Menu panels
		private FlowLayoutPanel upperMenu;
		private FlowLayoutPanel mainMenu;

		private Label nameOfSolarSystem;
		...
		/// <summary>
		/// Initializes GUI systrem for Mogre and loads fonts, skins and creates GUI overlay.
		/// </summary>
		/// <param Name="width">The window width</param>
		/// <param Name="height">The window height</param>
		/// <param Name="mouse">The Mogre mouse input</param>
		/// <param Name="keyboard">The mogre keyboard input</param>
		public GameGUI(int width, int height, MOIS.Mouse mouse, MOIS.Keyboard keyboard) {
			screenHeight = height;
			screenWidth = width;
		...
		/// <summary>
		/// Shows the groups properties and count or name of the object (if is just one in the group).
		/// </summary>
		...
		/// <summary>
		/// Prints the name as a current solar system name.
		/// </summary>
		...
		/// <summary>
		/// Disposes panel by the given name.
		/// </summary>
		...
		public void ShowTravelSelectionPanel(List<string> possibilities, object gameObject) {
			...
			// Creates SelectionLabels with names of the SolarSystems
		...
		/// <summary>
		/// Loads icons (GameActionIconBox) for each IGameAction in the given list and inserts them to gameActionPanel.
		/// </summary>
		...
		#region Top panel
		/// <summary>
		/// Creates top bar with label for a current name of the SolarSystem and ScrollablePanel for materials states.
		/// </summary>
		...
		/// <summary>
		/// Creates mainmenu subpanel. Initializes gameActionPanel for targeted group game actions and a game console.
		/// </summary>
		/// <returns>Returns initialized Panel with ScrollablePanel for group action and with a game console.</returns>
		...
		/// <summary>
		/// Closes all opened panels which are registed in openPanelDict and calls DestroyGame (destoy current mission).
		/// </summary>
		/// <param name="sender">The action sender.</param>
		/// <param name="e">The action argument.</param>
		...
		/// <param name="sender">The action sender.</param>
		/// <param name="e">The action argument.</param>
		/// <exception cref="Strategy.Exceptions.ShutdownException">Throws to close the application. Thrown always.</exception>
		...
		/// <exception cref="ShutdownException">Thrown when singleton PopUpPanel already exist.</exception>
		/// <returns>Returns created Panel with given text as topic.</returns>	
		private Panel CreatePopUpPanel(string text, PanelType type, bool isSingleton) {
			...
			if (isSingleton && openedPanelDict.ContainsKey(type.ToString())) {
				// The panel is already opened so it is invalid action and close application.
		...
		/// <summary>
		/// Creates a new singleton PopUpPanel with names of the cuurent saved mission and TextBox for insert the name of the save.
		/// </summary>
		...
		/// <summary>
		/// Creates panel with scroll bars, setts padding and border thickness.
		/// </summary>
		...
		private Label CreatePropertyLabel(int marginLeft, int marginTop, object property) {
			...
			MethodInfo method = typeof(GameGUI).GetMethod("CreatePropertyLabelGeneric", BindingFlags.NonPublic | BindingFlags.Instance);
		...
		/// </summary>
		/// <param Name="propertiesDict">The dictionary with properties (key - Name, value - Property)</param>
		...
		/// <summary>
		/// Creates an appropriate name for a panel (singleton - just returns the name, else generates the unique name).
		/// </summary>
		...
		/// <returns>Returs the unique name for the panel.</returns>
