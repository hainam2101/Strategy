...
namespace Strategy.TeamControl {
	...
	public class Team {
		...
		private Dictionary<string, IMaterial> materialsStates;

		private List<IMovableGameObject> imgoObjects;
		private List<IStaticGameObject> isgoObjects;
		private string name;
		...
		/// </summary>
		/// <param name="materialNeededDict">The dict with quantities of materials.</param>
		/// <exception cref="MissingMaterialException">Thrown when team has not enough materials.</exception>
		public void UseMaterials(Dictionary<string, int> materialNeededDict) {
			foreach (var materialPair in materialNeededDict) {
				...
				if (!materialsStates.ContainsKey(materialPair.Key) ||
					materialsStates[materialPair.Key].GetQuantityOfMaterial().Value < materialPair.Value) {
					// Team does not have the necessary quantity of material 
					throw new MissingMaterialException("You can not use " + materialsStates[materialPair.Key].Name + ", because you do not have enough materials.");
				}
				materialsStates[materialPair.Key].GetQuantityOfMaterial().Value -= materialPair.Value;
			}
		}
