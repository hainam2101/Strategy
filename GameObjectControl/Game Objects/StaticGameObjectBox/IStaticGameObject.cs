...
namespace Strategy.GameObjectControl.Game_Objects.StaticGameObjectBox {
	...
	public interface IStaticGameObject : IGameObject {
		...
		/// <summary>
		/// Moves with the object in visible mode (the object is in an active SolarSystem).
		/// </summary>
		/// <param name="f">The delay between last two frames (seconds).</param>
		void Rotate(float f);

		/// <summary>
		/// Moves with the object in invisible mode (the object is in a non-active SolarSystem). 
		/// </summary>
		/// <param name="f">The delay between last two frames (seconds).</param>
