...
namespace Strategy.GameObjectControl.Game_Objects.GameSave {
	...
	/// <summary>
	/// Serializes the current mission to a XML document.
	/// </summary>
	public class GameSerializer {
		...
		/// <summary>
		/// Creates the instance of the GameSerializer
		/// </summary>
		...
		private void SerializeTeamMaterials(XElement rootElement, Team team) {
			foreach (var material in team.GetMaterials()) {
				...
				SerializeArgument(element, material.Value.Quantity.ToString());
				rootElement.Add(element);
			}
		}
		...
		/// <param name="dict">The dictionary which will be filled.</param>
		/// <exception cref="System.ArgumentException">Thrown when exist two or more targeted fields with the same order number.</exception>
		private void AddToSortedDictionary(int number, string value, SortedDictionary<int, string> dict) {
			if (dict.ContainsKey(number)) {
				throw new ArgumentException("The field (property) " + value +
					" cannot have same order (" + number + "). The order must be unique.");
			} else {
